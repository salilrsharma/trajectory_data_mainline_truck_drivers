function f = new_get_trajectory_data_for_choice_models(id, trajectories, trajectories_lf, lanes, count, alpha)
    data = trajectories(id);
    index = [];

    % Here write code for selecting the indices until the start of LCE
    [lane_change_initiation, lane_change_completion, smooth_traj] = new_LC_timings(alpha, data.y_sm, 0.009);

    for i = 1:length(data.t)
        if rem(data.t(i),0.5) == 0 & (i < lane_change_initiation)
            index = [index; i];
        end
        if i == lane_change_initiation
            index = [index; i];
        end
    end
...
%     y_variable = [diff(local_veh_lanes); 1];
    y_variable = zeros(size(index,1),1);
